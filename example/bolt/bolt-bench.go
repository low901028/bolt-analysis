			...
			go func(id int) {
				...
				for j := 0; j < tasks; j++ {
					if rand.Intn(100) <= 20 {
						...
						//fmt.Println("the di is ", di, "the length is ", len(keys[di]))
...
func openDBs(paths []string) []*bolt.DB {
	var dbs []*bolt.DB
	for _, p := range paths {
		fmt.Println("the path is ", p)
		db, err := bolt.Open(p, 0600, &bolt.Options{Timeout: 1 * time.Second})
		if err != nil {
			log.Fatal("open the database file is failured ", err)
		}
		dbs = append(dbs, db)
	}
	return dbs
...
func listDBPaths(begin rune, disks int, partitions int) []string {
	...
	for i := 0; i < disks; i++ {
		for j := 0; j < partitions; j++ {
			path := fmt.Sprintf("F:\\bench\\sd%c\\", begin)
			//path := fmt.Sprintf("/srv/node/sd%c/", begin)
			db := fmt.Sprintf(path+"%d%d", i, j)
			if _, err := os.Stat(db); err != nil { // err ！= nil代表对应的目录不能存在
				if os.IsNotExist(err) { // 不存在
					if err = os.MkdirAll(path, os.ModePerm); err != nil {
						log.Fatal("create document is failured ", err)
					}
				}
			}

			//db := fmt.Sprintf("F:\\bench\\sd%c\\%d%d",begin, i, j)
	...
	for i, db := range dbs {
		...
		for j := 0; j < num; j++ {
			...
			if err := db.Batch(func(tx *bolt.Tx) error {
				...
				log.Printf("execute batch is failured %s", err)
			}
			ks = append(ks, p)
